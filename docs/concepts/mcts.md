...
## Virtual loss and batching

pymcts runs many games concurrently and batches all neural net evaluations into a single GPU call per simulation step.

**Virtual loss** makes this work: when a leaf is selected for evaluation, it's temporarily penalized (as if it lost) so that parallel selections pick different leaves. After the real evaluation comes back, the virtual loss is removed and the true value is backpropagated.

With `num_parallel_leaves=K`, each simulation step selects K different leaves per game, all evaluated in one batch. This gives K× fewer GPU round-trips.
